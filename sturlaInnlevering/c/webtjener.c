...
int main (){
    ...
    char *http_bad_request = "HTTP/1.1 404 File not found\n\nFile not found!\n";
    ...
    //chroot("/home/sturla/Desktop/github/daNanSturla/mp2/www/");
    chroot("/var/www/");   
    ...
    while(1){ 
        ...
        if(fork() == 0) {    
            ...
            //Sjekker først at filen eksisterer 
            if(fileExists(file_path) == 1 || file_path == "/"){
                //Finner filendelse
                ...
                if (file_type != NULL){             
                    ...
                    // text/plain ASIS
                    else if (strcmp(file_type, "asis") == 0){
                        file_pointer = fopen(file_path, "r"); 
                        if(file_pointer != NULL){
                            // Ingen header (er i filen)
                            while (fgets(http_response, BUFSIZ, file_pointer) != NULL) {
                                send(client_socket, http_response, strlen(http_response), 0);
                            }
                        }
                    ...
                    // text/plain
                    else if (strcmp(file_type, "asc") == 0 || strcmp(file_type, "txt") == 0 || strcmp(file_type, "text") == 0 || strcmp(file_type, "pot") == 0 || strcmp(file_type, "brf") == 0 || strcmp(file_type, "srt") == 0 || strcmp(file_type, "log") == 0){
                        file_pointer = fopen(file_path, "r"); 
                        if(file_pointer != NULL){
                            send(client_socket, text_html, strlen(text_html), 0);
                            while (fgets(http_response, BUFSIZ, file_pointer) != NULL) {
                    ...
                    // image/png
                        ...
                        if(file_pointer != NULL){
                            ...
                            while (fread(http_response, 1, sizeof(http_response), file_pointer) != 0) {
                ...
                fprintf(stderr, "%s - Filen finnes ikke\n", file_path);   
            }   
...
int fileExists(const char* filename){
    ...
    if(exist == 0)
        return 1; // Filen eksister
    else // filen eksister ikke
        return 0;
}
